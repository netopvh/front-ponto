...
const MenuItemWithChildren = ({ item, linkClassName, subMenuClassNames, activeMenuItems, toggleMenu }: SubMenus) => {
  ...
  return (
    <li className={`menu-item ${open ? 'menuitem-active' : ''}`}>
      <Link
        to='#'
        className={`${linkClassName} ${activeMenuItems!.includes(item.key) ? 'open' : ''}`}
        aria-expanded={open}
      ...
      >
        ...
        {!item.badge ? (
          ...
          <span className={`badge ${item.badge.variant}`}>{item.badge.text}</span>
        )}
      </Link>
  ...
  const activeMenu = useCallback(() => {
    ...
    if (div) {
      ...
      for (let i = 0; i < items.length; ++i) {
        let trimmedURL = location?.pathname?.replaceAll(process.env.VITE_BACKEND_URL || '', '')
        const url = items[i].pathname
        if (trimmedURL === process.env.VITE_BACKEND_URL + '/') {
          trimmedURL += 'ecommerce'
        }
        if (trimmedURL === url?.replaceAll(process.env.VITE_BACKEND_URL, '')) {
